...
const CartItem = ({
  ...
  const removeItem = () => {
    dispatch(removeProductToCart(uniq_id));
  };
  ...
  const incrementOrDecrement = (e, type) => {
    ...
    if (type === "inc" && value < 10) {
      ...
      dispatch(incrementCartQuantity(uniq_id));
    }

    if (type === "desc" && value > 1) {
      setItemQuantity(itemQuantity - 1);
      dispatch(decrementCartQuantity(uniq_id));
    }
  };
    ...
    <div className="row align-items-center mb-3">
      ...
      <div className="col-12 text-sm-center col-sm-12 text-md-left col-md-6">
        <h4 className="product-name">
          <strong>{shortenTitle(product_name)}</strong>
        </h4>
        <h4>
